...
bookReturnRouter.post("/", async (req: Request, res: Response) => {
    ...
    const { returned_book_id, selected_books_to_return } = req.body;
    let totalLateFee = 0;
    let status = 'Completed';

    try {
        console.log(returned_book_id)
        const regex = new RegExp(returned_book_id + '$');
        console.log(regex)
        ...
        if (selected_books_to_return.length != bookRent.borrowed_books.length) {
            status = 'Partial'; 
        }
